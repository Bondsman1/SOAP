...
#if defined(NDEBUG)
# error "Soap cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Soap Signed Message:\n";
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        ...
        // keys. (remSoap the 520 byte limit on redeemScript size) That works
...
int64_t GetProofOfWorkReward(int64_t nHeight, int64_t nFees)
{
    ...
    if(nHeight == 3)
    {
        nSubsidy = 1000000 * COIN; // 2% Premine
    }
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
        ...
        int64_t PastBlocksMax = 110; // Allow min difficulty during start of chain
